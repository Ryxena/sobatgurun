                    ...
                    <div class="modal-body">
                        <form action="mahasiswa/tambah" method="post">
                            ...
                            <div class="mb-3" id="date">
                                ...
                                <input type="date" class="form-control" name="tanggal_daftar" id="tgl_daftar"
                                       value="<?= date('Y-m-d') ?>">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="form-floating">
                                <textarea name="alamat" class="form-control" placeholder="Leave a comment here"
                                          id="alamat" style="height: 100px"></textarea>
        ...
        <ul class="list-group">
            <?php foreach ($data["mahasiswa"] as $siswa) : ?>
                <li class="list-group-item">
                    <?= $siswa["nama_lengkap"] ?>
                    <a href="<?= BASE_URL ?>/mahasiswa/delete/<?= $siswa["id_siswa"] ?>"
                       class="btn btn-danger float-end m-2" onclick="return confirm('yakin ?');">hapus</a>
                    <button class="btn btn-success float-end m-2 tampilModalUbah" data-bs-toggle="modal"
                            data-bs-target="#exampleModal" data-id="<?= $siswa["id_siswa"] ?>">Ubah
                    </button>
                    <a href="<?= BASE_URL ?>/mahasiswa/detail/<?= $siswa["id_siswa"] ?>"
                       class="btn btn-danger float-end m-2">detail</a>
                </li>
            <?php endforeach; ?>
        </ul>
    </div>
